"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.updateProduct = void 0;
const https_1 = require("firebase-functions/v2/https");
const admin = require("firebase-admin");
const validators_1 = require("../utils/validators");
exports.updateProduct = (0, https_1.onCall)(async (request) => {
    var _a, _b;
    const { auth, data } = request;
    if (!auth || !auth.uid) {
        throw new https_1.HttpsError('unauthenticated', '로그인이 필요합니다.');
    }
    if (auth.token.role !== 'brand') {
        throw new https_1.HttpsError('permission-denied', '브랜드 계정만 제품을 수정할 수 있습니다.');
    }
    const productId = data.productId;
    if (!productId) {
        throw new https_1.HttpsError('invalid-argument', '제품 ID가 누락되었습니다.');
    }
    try {
        (0, validators_1.validateProductInput)(data);
    }
    catch (error) {
        throw new https_1.HttpsError(error.code || 'invalid-argument', error.message || '잘못된 입력입니다.');
    }
    try {
        const db = admin.firestore();
        const productRef = db.collection('products').doc(productId);
        const doc = await productRef.get();
        if (!doc.exists) {
            throw new https_1.HttpsError('not-found', '제품을 찾을 수 없습니다.');
        }
        if (((_a = doc.data()) === null || _a === void 0 ? void 0 : _a.brandId) !== auth.uid) {
            throw new https_1.HttpsError('permission-denied', '자신의 제품만 수정할 수 있습니다.');
        }
        if (((_b = doc.data()) === null || _b === void 0 ? void 0 : _b.status) === 'deleted') {
            throw new https_1.HttpsError('failed-precondition', '삭제된 제품은 수정할 수 없습니다.');
        }
        await productRef.update({
            name: data.name,
            category: data.category,
            description: data.description,
            price: data.price,
            images: data.images,
            updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        });
        return { success: true };
    }
    catch (error) {
        console.error('제품 수정 실패:', error);
        if (error instanceof https_1.HttpsError)
            throw error;
        throw new https_1.HttpsError('internal', '제품 수정에 실패했습니다.');
    }
});
//# sourceMappingURL=updateProduct.js.map