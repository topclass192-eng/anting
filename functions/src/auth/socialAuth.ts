...
export interface ISocialAuthProvider {
  ...
  getUserInfo(token: string): Promise<ISocialUser>;
}

export async function createFirebaseToken(socialUser: ISocialUser): Promise<{ firebaseToken: string, isNewUser: boolean }> {
